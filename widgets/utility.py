...
class Utility:
	...
	def validate_birthday(widget, error_color, success_color, at_least_years_user, error_callback):
		...
		try:
			...
			if birth_date > current_date:
				Utility.set_color(widget, error_color)
				error_callback('', "Birthday cannot be a future date. Please enter a valid date.")
				return False
			...
			if age < at_least_years_user:
				Utility.set_color(widget, error_color)
				error_callback('', f"You must be at least {at_least_years_user} years old to enter.")
				return False
			...
			Utility.set_color(widget, error_color)
			error_callback('', "Invalid date format. Please use MM/DD/YYYY.")
			return False
	...
	def validate_email(widget, email_pattern, error_color, success_color, error_callback):
		...
		if email == "":
			Utility.set_color(widget, error_color)
			error_callback('', "Email cannot be empty. Please enter a valid email address.")
			return False
		elif not re.match(email_pattern, email):
			Utility.set_color(widget, error_color)
			error_callback('', "Invalid email format. Please enter a valid email address.")
			return False
		Utility.set_color(widget, success_color)
		return True
	...
	def validate_password(widget, confirm_widget, error_color, success_color, error_callback, check_strength=False):
		...
		if password == "":
			Utility.set_color(widget, error_color)
			error_callback('', "Password cannot be empty. Please enter a valid password.")
			return False
		if check_strength and not Utility.is_strong_password(password):
			Utility.set_color(widget, error_color)
			error_callback('', "Password is too weak. It must be at least 8 characters long, include a mix of uppercase and lowercase letters, numbers, and special characters.")
			return False
		if confirm_widget.input.text.strip() != password:
			Utility.set_color(widget, error_color)
			Utility.set_color(confirm_widget, error_color)
			error_callback('', "Passwords do not match. Please ensure both passwords are identical.")
			return False
		Utility.set_color(widget, success_color)
